...
class quantum_two_level_dynamics:
	def __init__(self, pulse_sequencer, readout_device, ex_channel, ro_channel, ro_sequence, ex_amplitude, readout_measurement_name, qubit_id, sample_name, comment, shuffle=False, **kwargs):
	...
	def Rabi_rect(self,lengths, tail_length=0e-9, pre_pulse_seq=[]):
		...
		state = sweep.sweep(self.readout_device, (lengths, set_ex_length, 'Rabi pulse length, s'))
		state.measurement_type = 'Rabi'
		state.sample_name = self.sample_name
		state.comment = self.comment
		state.metadata = {'Amplitude': str(self.ex_amplitude), 'Qubit id': str(self.qubit_id)}
	...
	def Ramsey(self,delays,target_freq_offset,*params,pre_pulse_seq=[], cross_ex_device=None, dump=True):
		...
		state = sweep.sweep(self.readout_device, (delays, set_delay, 'Ramsey delay', 's'))
		state.measurement_type = 'Ramsey'
		state.sample_name = self.sample_name
		state.comment = self.comment
		state.metadata = {'Amplitude': str(self.ex_amplitude), 'Qubit id': str(self.qubit_id)}
		fitted_data, fitted_parameters = self.fitter(delays, state.data, fitting.exp_sin_fit)
		state_fitted = state.copy()
		state_fitted.references = {state.id: 'fit'}
		state_fitted.metadata.update({'Frequency': str(fitted_parameters['freq']), 'Decay': str(fitted_parameters['decay']), 'Phase': str(fitted_parameters['phase']), 'Amplitude': str(fitted_parameters['amplitudes']), 
										'Target offset': str(target_freq_offset), 'Excitation channel': str(ex_channel), 'Readout channel': str(ro_channel)})
		for dataset in state_fitted.datasets.keys():
			state_fitted.datasets[dataset].data = fitted_data
		save_exdir(state_fitted)
		db = database.database()
